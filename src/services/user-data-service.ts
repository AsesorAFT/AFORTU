...
export async function fetchUserPortfolio(): Promise<string> {
  ...
  const portfolioItems = portfolioSnapshot.docs.map(
    (doc) => doc.data() as PortfolioItem,
  );
  const portfolioDescription = portfolioItems
    .map(
      (item) =>
...
const accountLogData = [
  {
    date: "2024-07-29",
    activity: "Revisión trimestral de portafolio",
    advisor: "Asesor AFT",
    details: "Se ajustó la ponderación en acciones tecnológicas.",
  },
  {
    date: "2024-07-15",
    activity: "Llamada de seguimiento",
    advisor: "Asesor AFT",
    details: "Se discutieron las proyecciones económicas para el Q3.",
  },
];
