...
export async function initializeUserData(user: User, isPro: boolean) {
  ...
  const defaultStructure = {
    ...
    advisor: {
      ...
      systemPrompt:
        "Eres Asesor AFT, un experto financiero. Sé amable y proactivo.",
      quickActions: [
        {
          label: "Analizar portafolio",
          prompt: "Analiza mi portafolio actual.",
        },
      ],
    },
...
export const signInWithGoogle = async (isAfortuPro = false) => {
  if (!auth) throw new Error("Firebase auth not configured");
  ...
  try {
    ...
    if (additionalInfo?.isNewUser) {
      await initializeUserData(result.user, isAfortuPro);
    }
    return result;
  } catch (error: any) {
